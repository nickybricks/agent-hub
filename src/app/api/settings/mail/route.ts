...
function redactSecrets(mail: MailConfig | undefined): MailConfig & { _status: Record<string, boolean> } {
  const m = mail ?? {};
  return {
    provider: m.provider ?? "imap",
    imap: m.imap
      ? { host: m.imap.host, port: m.imap.port, user: m.imap.user, password: undefined }
      : undefined,
    gmail: m.gmail
...
export async function GET() {
  const full = readFull();
  const mail = (full.mail as MailConfig | undefined);
  return NextResponse.json(redactSecrets(mail));
}
...
export async function PUT(request: Request) {
  ...
  // Merge so blank secret fields don't wipe stored secrets.
